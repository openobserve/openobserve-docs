...
OpenObserve provides ability to index data in multiple ways to make it faster. It uses a combination of partitioning, bloom filters, inverted indexes, caching and columnar storage to make search and aggregation queries faster. Using the above combination you can balance between storage and performance, which is still typically much higher than most alternative systems. Aggregation queries are typically much faster than Elasticsearch and other systems due to columnar storage. [Uber found 80% of queries in their production environment to be aggregation queries](https://www.uber.com/en-IN/blog/logging/) and columnar data storage of OpenObserve means that aggregation queries will typically be much faster than Elasticsearch.

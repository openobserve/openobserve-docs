---
description: OpenObserve Enterprise Edition pricing, license tiers, and usage limits — free up to 50 GB/day, paid above that.
---
### Enterprise License

Enterprise Edition is free for up to 50 GB/day (~1.5 TB/month) of data ingestion

#### Pricing:

  - Free tier: Up to 50 GB/day of ingestion (roughly 1.5 TB/month), including full commercial use in accordance with the [license](https://openobserve.ai/enterprise-license/)

  - Registration required if ingesting greater than 10 GB/day.

  - Volume discounts and multi-year contracts available, [contact our sales team](https://openobserve.ai/contact-sales/)

---

#### How Enterprise Licensing Works

Enterprise installations work without requesting a license key up to **10 GB/day** of data ingestion. If your ingestion volume goes beyond 10 GB/day, you need to request a license for your team.

##### License Types

1. **Free-Tier License** (≤ 50 GB/Day)
2. **Paid Subscription License** (> 50 GB/Day)

##### Usage Limits

To accommodate occasional traffic spikes, OpenObserve provides a buffer:

- You can exceed your license limit up to **3 times per month** without impact.
- After exceeding your limit 3 times in a calendar month, search queries will be blocked until you upgrade your license or your ingestion volume returns within limits.

This applies to both fresh installations (10 GB/day limit) and Free-Tier License users (50 GB/day limit).

For full terms and conditions, visit the [Enterprise License page](https://openobserve.ai/enterprise-license/).

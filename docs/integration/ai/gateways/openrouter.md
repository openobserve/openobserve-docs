...
Call `OpenAIInstrumentor().instrument()` **before** creating the OpenAI client. Pass the two optional OpenRouter headers via `default_headers` — these are not required for requests to succeed, but they populate your OpenRouter usage dashboard.
...
Free-tier models use the `:free` suffix. OpenRouter's full model list is available at [openrouter.ai/models](https://openrouter.ai/models).

## **What Gets Captured**

| Attribute | Description |
| ----- | ----- |
| `llm_model_name` | Full model identifier including provider (e.g. `meta-llama/llama-3.1-8b-instruct:free`) |

...
1. An [OpenObserve Cloud](https://cloud.openobserve.ai) account or OpenObserve [Self-Hosted](../../../getting-started.md#self-hosted-installation) setup.
...
:::accordion[Get OpenObserve Credentials]
...
:::accordion[Create ECS task definition]

1. Create the following file and save it as "nginx_firelens_zo_task_def.json"
```json title="ECS task definition - nginx_firelens_zo_task_def.json" lineNumbers {4,24,44-54}
{
...
2. Register the task definition using the below command:
  ...
  ```json title="Network configuration - network_config.json" lineNumbers {10}
  {
    "awsvpcConfiguration": {

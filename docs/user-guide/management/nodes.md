...
In the OpenObserve UI, go to **Management > Nodes** from the top navigation menu.  
...
- It is accessible only under the `_meta` organization.  
- By default, only admin users can view this page. To grant access to other roles, use [Role-Based Access Control (RBAC)](https://openobserve.ai/docs/user-guide/identity-and-access-management/enable-rbac-in-openobserve-enterprise/) to assign the necessary permissions.

...
=== "Overview"
    ...
    !!! note "Where to find it"
        ...
        ![Traces](../../../ingestion/traces/images/traces.png)
    ...
    **Sessions list:** Groups LLM traces into conversations so you can review each session's user, first message, turns, duration, tokens, cost, and status in a single row.

    ## Sessions list

    The **Sessions** view of the **Traces** page groups traces into LLM sessions. A session is one conversation — all traces (turns) that share the same session/conversation ID — and the list shows one row per session.

    ![TODO: screenshot of the Sessions list showing the search box and sortable columns](images/placeholder.png)

    Each row shows:

    - **User** and the **first user message** of the conversation
    - **Turns** (the number of traces in the session)
    - **Duration** and **Last activity**
    - **Tokens** (input → output = total) and **Cost**
    - **Status**, marked as error if any span in the session failed

    ### Search sessions

    Use the search box above the list to filter sessions by user or conversation text. Type a term and the list re-filters automatically (no need to press Enter) after a short 300 ms debounce.

    ![TODO: screenshot of the "No sessions match your search" empty state](images/placeholder.png)

    - The term is matched case-insensitively against the session's **user ID** or its **conversation text** (the input and output messages of any span in the session).
    - Search is literal text — no wildcards or operators — and is combined with your current stream or agent filter using **AND**.
    - Terms longer than 256 characters are truncated automatically.
    - Select the **x** in the box, or **Clear search** on the no-results screen, to remove the filter.
    - A filtered view is shareable: the term is kept in the URL as `?keyword=<term>`, and navigating into a session and back restores the filtered list.

    ### Sort sessions

    Click any sortable column header to order the list. Sorting is performed server-side across the entire result set, so the order is correct across all pages — not just the page you are viewing.
    ...
    Invalid sort requests are rejected by the server, and the list always falls back to a deterministic order.
=== "How-to"
    <br>

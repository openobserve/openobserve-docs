...
!!! note "Versions"
    ...
    The mobile SDKs are on their first stable releases — React Native `0.1.1`, Android `0.1.0`, iOS `0.1.0`. The performance metrics described here are collected today; pin your exact SDK version and re-test on upgrades, since defaults and option names may still change across early `0.1.x` releases.
